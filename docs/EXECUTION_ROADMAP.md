...
**Current milestone:** Milestone 2.3 — Repository-grounded RAG (not started)
**Status:** Release 1 — Product foundation is complete (Milestones 0.1 through 1.6; Jesse approved Release 1 as complete). Milestone 2.1 — Durable asynchronous workflow is complete (Jesse approved). Milestone 2.2 — Provider-neutral AI gateway is complete (Jesse approved): [ADR 0008](adr/0008-provider-neutral-ai-gateway.md) adds `app/ai_gateway/` — a provider-neutral gateway with validated `AIRequest`/`AIResponse` contracts — and wires it into the existing Celery workflow (ADR 0007) as an additive `ai_inference` stage between the deterministic `propose` and `human_review` stages; the primary recommendation (classification, severity, proposed action, evidence) stays entirely deterministic and unchanged, and the AI narrative only ever supplements it, never replacing evidence or the explicit human decision. `AI_PROVIDER` defaults to `mock` (deterministic, zero network, zero cost) everywhere — tests, CI, and the normal demo; OpenAI is the single real adapter, opt-in only via `AI_PROVIDER=openai` plus `OPENAI_API_KEY`, built on the project's existing `httpx` dependency (promoted from dev-only to a runtime dependency; no new dependency). The request sent to any provider carries only bounded, already-sanitized, already-deterministic fields (classification, evidence, assessment, proposed action) — nothing raw and nothing newly retrieved. A provider timeout, HTTP error, or malformed/missing response falls back to the mock adapter within the same stage attempt with a recorded reason, spending no extra Celery retry; an `AI_PROVIDER=openai` misconfiguration (no key) fails worker/API startup immediately instead of silently falling back. Every result records provider, model, prompt name, input/output tokens, estimated cost, and latency; estimated cost is computed from configurable per-million-token price settings, and no output-token cap is imposed. No database migration: reuses `stage_attempts.output`, `Recommendation.content`, and a new `AuditEvent` type (`ai_inference_routing`) recording the routing/fallback decision, written only once per attempt. Milestone 2.1's per-stage resume mechanism was reused unchanged for the new stage, so a redelivered/resumed task never repeats a successful (paid or mock) provider call — proven directly by test. Verified: 139 backend tests passed against a fresh isolated, disposable PostgreSQL database (no migration to validate), ruff format/lint clean, 18 frontend tests passed (lint and build clean), Compose configuration validated, and the demo database remained exactly 100 fixture issues, untouched by any automated test. A visible browser demo showed the separate AI-generated inference section, `Provider: mock (deterministic-v1)`, unchanged deterministic results, and intact human-review controls. No real, paid OpenAI call was made at any point in implementation, verification, or the demo; the OpenAI adapter was deliberately verified using mocked provider responses only, and real-provider smoke verification remains separately authorized for later. Jesse reviewed the visible demo and explicitly approved Milestone 2.2 and R2-02 as complete.
**Last approved decision:** Milestone 2.2 — Provider-neutral AI gateway (ADR 0008: mock-default, OpenAI opt-in, additive `ai_inference` stage) approved complete by Jesse.
**Next action:** Begin Milestone 2.3 — Repository-grounded RAG.
