...
**Current milestone:** Milestone 2.2 — Provider-neutral AI gateway (not started)
**Status:** Release 1 — Product foundation is complete (Milestones 0.1 through 1.6; Jesse approved Release 1 as complete). Milestone 2.1 — Durable asynchronous workflow is complete: [ADR 0007](adr/0007-durable-async-workflow-technology.md) selects Celery (worker) + Redis (broker/result backend) with PostgreSQL as the sole canonical workflow-state store, superseding the deferred [ADR 0004](adr/0004-defer-async-workflow-technology-selection.md). Verified evidence: genuine asynchronous execution — the five stages (classify → retrieve_fixture_evidence → assess → propose → human_review) run in a Celery task against a real, non-eager Redis broker, where `self.retry()` was proven to produce independent broker-mediated task redeliveries (not eager-mode in-process recursion); PostgreSQL remains the sole canonical workflow-state store (migration `20260914_0002` adds `analyses.idempotency_key`/`current_stage`/`attempt_count` and the `stage_attempts` table; the additive migration `20260914_0003` adds `stage_attempts.output`, persisting each stage's deterministic result, plus a database-level `uq_stage_attempts_analysis_stage_attempt` uniqueness constraint — kept as its own migration rather than editing the already-applied `0002`, so the fix applies safely to both a fresh database and one that had already migrated); `POST /api/v1/issues/{issue_id}/triage` returns `202 Accepted` with an analysis id/status/poll URL, and `GET` exposes `current_stage`, `attempt_count`, and stage-attempt history; `Idempotency-Key` deduplicates same-issue requests and rejects cross-issue reuse (409); real, not simulated, crash and timeout recovery — a genuinely killed worker and a real elapsed stage timeout were exercised against isolated Docker infrastructure, and a redelivered task now resumes at the first incomplete stage rather than re-running or duplicating already-succeeded `StageAttempt`/`Recommendation`/`HumanDecision` rows; the React UI submits once and polls with a bounded 30-second automatic cap, and reaching that cap while the workflow is still non-terminal pauses automatic polling without re-enabling "Run deterministic triage" — offering a GET-only "Refresh status" action instead that never starts a second workflow. The worker never creates a `HumanDecision`; the explicit Approve/Reject/Request revision gate is unchanged. Final verification: 113 backend tests passed against a fresh isolated, disposable PostgreSQL database (migration upgrade/downgrade/upgrade validated across 0001→0002→0003), ruff format/lint clean, 17 frontend tests passed with lint and build clean, and the demo database remained exactly 100 fixture issues throughout every verification and recovery exercise. Jesse reviewed and explicitly approved Milestone 2.1 and R2-01 as complete.
**Last approved decision:** Milestone 2.1 — Durable asynchronous workflow (ADR 0007: Celery + Redis + PostgreSQL) approved complete by Jesse.
**Next action:** Begin Milestone 2.2 — Provider-neutral AI gateway.
