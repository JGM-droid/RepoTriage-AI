...
**Current milestone:** Milestone 2.4 — Prompt registry and traceability (not started)
**Status:** Release 1 — Product foundation is complete. Milestone 2.1, Milestone 2.2, and Milestone 2.3 are complete (Jesse approved all three). Milestone 2.3 — Repository-grounded RAG: [ADR 0009](adr/0009-repository-grounded-pgvector-retrieval.md) makes PostgreSQL + `pgvector` the primary vector store (image `pgvector/pgvector:0.8.6-pg17-bookworm`, verified safe against the existing demo volume — same major version, matching collation, all data intact through the swap) and adds one additive migration (`20260916_0004`: `repository_documents`, `retrieval_chunks` with a `vector(384)` column; migrations 0001-0003 untouched). Embeddings use a real, local, CPU-only model (`fastembed` 0.8.0 + `BAAI/bge-small-en-v1.5`, MIT licensed) baked into the Docker image at build time — no paid API call, no runtime network access; tests use a separate deterministic fake adapter. A new `retrieve_related_evidence` Celery stage runs between the deterministic `propose` and `ai_inference` stages through the existing `_run_stage`, inheriting Milestone 2.1's persisted-output/resume/no-duplicate-call guarantees unchanged — a redelivered task never re-queries or re-embeds. Retrieval is strictly repository-scoped, excludes the analyzed issue itself, deduplicates to each source's best chunk, and returns at most 3 results as a cap, never a quota — fewer than 3 accepted candidates is a valid, expected outcome, and rejected candidates are never backfilled to fill out the slate. Only the selected, bounded top-k results reach the AI narrative's prompt; the AI-generated citations are validated against the supplied identifiers, with an invented citation treated as a controlled provider failure using the existing fallback path (no new mechanism). All 100 existing resolved issues plus a new small, attributed, checksum-verified documentation fixture (6 entries, real Flask-docs source URLs) were chunked and embedded via an idempotent `python -m app.retrieval` command — verified idempotent by repeated runs performing zero re-embeds. The retrieval query itself is built from issue-specific content only (normalized title, a bounded body excerpt, deduplicated matched keywords) — generic classification/severity/rationale/proposed-action boilerplate is deliberately excluded from the embedded query (though still available unchanged to the `ai_inference` stage), because that boilerplate was found to homogenize similarity scores across unrelated issues and defeat any similarity threshold. Acceptance uses a two-band confidence model, not a single cutoff (this model, and its exact calibrated thresholds, postdate and supersede the original threshold discussion in ADR 0009 — see `tests/calibration/bge_similarity_calibration.json` for the current, authoritative measured evidence): below `retrieval_min_similarity` (0.65, calibrated against the real production query builder and this corpus) a candidate is rejected outright; from 0.65 up to `retrieval_high_confidence_similarity` (0.90, calibrated from the confirmed near-duplicate #5755/#5756 pair measuring ~0.928) a candidate additionally needs at least one shared *discriminative* term with the query (`app.retrieval.query_terms.extract_discriminative_terms` — meaningful terms stripped of corpus-generic boilerplate words like "flask"/"python"/"code"/"environment"/"version", a small bounded set of general low-content English words like pronouns/modals/"using", and standalone numbers); at or above 0.90 a candidate is accepted on semantic score alone. This two-band model, and the discriminative-term filter specifically, replaced an earlier single narrow "close case" margin after Jesse's visible demo surfaced a false positive (issue #5942, an unrelated packaging request, scored just above that margin's ceiling and was accepted on semantic score alone); building and live-verifying the correction against the real demo corpus caught and fixed two further latent false positives (issues #5804 and #5836, both accepted only via shared corpus-boilerplate/low-content words) before they ever reached a real demo. One honest, disclosed limitation remains and was not force-fixed: issue #6139 ("AI junk", a demo-corpus filler issue that genuinely discusses security vulnerabilities) is accepted for #5755 via the shared discriminative term "security" — a different sense of "security" than #5755's URL-path name, illustrating that single-shared-term lexical confirmation cannot fully resolve polysemy. Verified: 217 backend tests passed against a fresh isolated pgvector PostgreSQL database (migration upgrade/downgrade/upgrade validated from an empty database across 0001→0004→0003→0004), ruff format/lint clean, 19 frontend tests passed (lint and build clean; no frontend code changed in the final correction), Compose configuration validated, and the demo database remained exactly 100 issues and 405 retrieval chunks throughout every image swap, migration, and correction. Jesse's own visible demo run for issue #5755 (the actual persisted `Recommendation` row, not a rehearsal) returned exactly `["issue:5756", "issue:5863", "issue:6139"]` as citations — #5756 at cosine similarity 0.928 (accepted on the strong-confidence threshold alone), #5863 at 0.708 (accepted via the shared term "security"), #6139 at 0.691 (the disclosed limitation above) — with #5942 correctly absent, unchanged deterministic classification/severity/proposed action, intact human-review controls, and zero external/paid network calls (confirmed via container logs and `provider: "mock"` in the persisted record). Jesse reviewed this demo and explicitly approved Milestone 2.3 and R2-03 as complete.
**Last approved decision:** Milestone 2.3 — Repository-grounded RAG (ADR 0009: pgvector + local BGE embeddings; post-implementation calibration corrections recorded in this Current Project State entry and `tests/calibration/bge_similarity_calibration.json`) approved complete by Jesse.
**Next action:** Begin Milestone 2.4 — Prompt registry and traceability.
